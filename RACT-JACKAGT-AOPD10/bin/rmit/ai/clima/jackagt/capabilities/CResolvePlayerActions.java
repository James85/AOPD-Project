/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*
  This code is generated by JAC version 5.4 by
  Agent Oriented Software. http://www.agent-software.com.au

    DO NOT ALTER THIS CODE AND DO NOT REMOVE THIS COMMENT
 *=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
package rmit.ai.clima.jackagt.capabilities;
import aos.jack.jak.agent.Capability;
import aos.jack.jak.agent.NameSpace;
import aos.jack.jak.agent.Agent;
import rmit.ai.clima.jackagt.data.BPlayerPosition;
import rmit.ai.clima.jackagt.data.BPlayerGold;
import rmit.ai.clima.jackagt.data.BGoldAt;
import rmit.ai.clima.jackagt.data.BMoveHint;
import rmit.ai.clima.jackagt.data.SimulationProp;
import rmit.ai.clima.jackagt.data.BPlayerClosestGold;
import rmit.ai.clima.jackagt.data.BObstacleAt;
import rmit.ai.clima.jackagt.events.EFindPath;
import rmit.ai.clima.jackagt.events.MEPlayerAction;
import rmit.ai.clima.jackagt.events.EIdentifyTarget;
import rmit.ai.clima.jackagt.events.EFindClosestGold;
import rmit.ai.clima.jackagt.events.EPostPlayerAction;
import rmit.ai.clima.jackagt.plans.PSendPlayerToDepot;
import rmit.ai.clima.jackagt.plans.PTellPlayerToMove;
import rmit.ai.clima.jackagt.plans.PSendPlayerToGold;
import rmit.ai.clima.jackagt.plans.PTellPlayerToDrop;
import rmit.ai.clima.jackagt.plans.PTellPlayerToPick;
import rmit.ai.clima.jackagt.plans.PSendPlayerToExplore;

public class CResolvePlayerActions extends aos.jack.jak.agent.Capability {
    public rmit.ai.clima.jackagt.data.BPlayerPosition bel_playerTarget_dat;
    public rmit.ai.clima.jackagt.data.BPlayerPosition bel_playerPositions_dat;
    public rmit.ai.clima.jackagt.data.BPlayerGold bel_playerGold_dat;
    public rmit.ai.clima.jackagt.data.BGoldAt bel_goldAt_dat;
    public rmit.ai.clima.jackagt.data.BMoveHint bel_moveHint_dat;
    public rmit.ai.clima.jackagt.data.SimulationProp bel_simulationProp_dat;
    public rmit.ai.clima.jackagt.data.BPlayerClosestGold bel_playerClosestGold_dat;
    public rmit.ai.clima.jackagt.data.BObstacleAt bel_obstacleAt_dat;
    public rmit.ai.clima.jackagt.events.EFindPath efindpath_p;
    public rmit.ai.clima.jackagt.events.MEPlayerAction meplayeraction_p;
    public rmit.ai.clima.jackagt.events.EIdentifyTarget eidentifytarget_p;
    public rmit.ai.clima.jackagt.events.EFindClosestGold efindclosestgold_p;
    // Inner capabilities are declared here.
/******** End PDT Design Block *** DO NOT EDIT IT *********/
//other Data Member and Method definitions.
/* Constructor */

    public CResolvePlayerActions()
    {
        super();
    }
    
    public void __init1()
    {
        setNamedCreator("bel_playerTarget_dat","rmit.ai.clima.jackagt.data.BPlayerPosition",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_playerTarget_dat();
            }
            
        },true);
        setNamedCreator("bel_playerPositions_dat","rmit.ai.clima.jackagt.data.BPlayerPosition",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_playerPositions_dat();
            }
            
        },true);
        setNamedCreator("bel_playerGold_dat","rmit.ai.clima.jackagt.data.BPlayerGold",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_playerGold_dat();
            }
            
        },true);
        setNamedCreator("bel_goldAt_dat","rmit.ai.clima.jackagt.data.BGoldAt",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_goldAt_dat();
            }
            
        },true);
        setNamedCreator("bel_moveHint_dat","rmit.ai.clima.jackagt.data.BMoveHint",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_moveHint_dat();
            }
            
        },true);
        setNamedCreator("bel_simulationProp_dat","rmit.ai.clima.jackagt.data.SimulationProp",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_simulationProp_dat();
            }
            
        },true);
        setNamedCreator("bel_playerClosestGold_dat","rmit.ai.clima.jackagt.data.BPlayerClosestGold",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_playerClosestGold_dat();
            }
            
        },true);
        setNamedCreator("bel_obstacleAt_dat","rmit.ai.clima.jackagt.data.BObstacleAt",new aos.jack.jak.agent.DataCreator(false){
            public java.lang.Object create()
            {
                return __named_data_bel_obstacleAt_dat();
            }
            
        },true);
    }
    
    public void __init2()
    {
        bel_playerTarget_dat = (rmit.ai.clima.jackagt.data.BPlayerPosition) getNamedObject("bel_playerTarget_dat","rmit.ai.clima.jackagt.data.BPlayerPosition");
        bel_playerPositions_dat = (rmit.ai.clima.jackagt.data.BPlayerPosition) getNamedObject("bel_playerPositions_dat","rmit.ai.clima.jackagt.data.BPlayerPosition");
        bel_playerGold_dat = (rmit.ai.clima.jackagt.data.BPlayerGold) getNamedObject("bel_playerGold_dat","rmit.ai.clima.jackagt.data.BPlayerGold");
        bel_goldAt_dat = (rmit.ai.clima.jackagt.data.BGoldAt) getNamedObject("bel_goldAt_dat","rmit.ai.clima.jackagt.data.BGoldAt");
        bel_moveHint_dat = (rmit.ai.clima.jackagt.data.BMoveHint) getNamedObject("bel_moveHint_dat","rmit.ai.clima.jackagt.data.BMoveHint");
        bel_simulationProp_dat = (rmit.ai.clima.jackagt.data.SimulationProp) getNamedObject("bel_simulationProp_dat","rmit.ai.clima.jackagt.data.SimulationProp");
        bel_playerClosestGold_dat = (rmit.ai.clima.jackagt.data.BPlayerClosestGold) getNamedObject("bel_playerClosestGold_dat","rmit.ai.clima.jackagt.data.BPlayerClosestGold");
        bel_obstacleAt_dat = (rmit.ai.clima.jackagt.data.BObstacleAt) getNamedObject("bel_obstacleAt_dat","rmit.ai.clima.jackagt.data.BObstacleAt");
        efindpath_p = (rmit.ai.clima.jackagt.events.EFindPath) findEvent("rmit.ai.clima.jackagt.events.EFindPath");
        meplayeraction_p = (rmit.ai.clima.jackagt.events.MEPlayerAction) findEvent("rmit.ai.clima.jackagt.events.MEPlayerAction");
        eidentifytarget_p = (rmit.ai.clima.jackagt.events.EIdentifyTarget) findEvent("rmit.ai.clima.jackagt.events.EIdentifyTarget");
        efindclosestgold_p = (rmit.ai.clima.jackagt.events.EFindClosestGold) findEvent("rmit.ai.clima.jackagt.events.EFindClosestGold");
        autorun();
    }
    
    synchronized public void __init_desc()
    {
        externals.put("rmit.ai.clima.jackagt.events.EPostPlayerAction","rmit.ai.clima.jackagt.events.EPostPlayerAction");
        externals.put("rmit.ai.clima.jackagt.events.EFindClosestGold","rmit.ai.clima.jackagt.events.EFindClosestGold");
        externals.put("rmit.ai.clima.jackagt.events.EIdentifyTarget","rmit.ai.clima.jackagt.events.EIdentifyTarget");
        externals.put("rmit.ai.clima.jackagt.events.MEPlayerAction","rmit.ai.clima.jackagt.events.MEPlayerAction");
        externals.put("rmit.ai.clima.jackagt.events.EFindPath","rmit.ai.clima.jackagt.events.EFindPath");
        externals.put("bel_obstacleAt_dat","bel_obstacleAt_dat");
        externals.put("bel_playerClosestGold_dat","bel_playerClosestGold_dat");
        externals.put("bel_simulationProp_dat","bel_simulationProp_dat");
        externals.put("bel_moveHint_dat","bel_moveHint_dat");
        externals.put("bel_goldAt_dat","bel_goldAt_dat");
        externals.put("bel_playerGold_dat","bel_playerGold_dat");
        externals.put("bel_playerPositions_dat","bel_playerPositions_dat");
        externals.put("bel_playerTarget_dat","bel_playerTarget_dat");
        addEvent("rmit.ai.clima.jackagt.events.EFindPath",aos.jack.jak.agent.Agent.POSTED_EVENT);
        addEvent("rmit.ai.clima.jackagt.events.MEPlayerAction",aos.jack.jak.agent.Agent.POSTED_EVENT);
        addEvent("rmit.ai.clima.jackagt.events.EIdentifyTarget",aos.jack.jak.agent.Agent.POSTED_EVENT);
        addEvent("rmit.ai.clima.jackagt.events.EFindClosestGold",aos.jack.jak.agent.Agent.POSTED_EVENT);
        addEvent("rmit.ai.clima.jackagt.events.EPostPlayerAction",aos.jack.jak.agent.Agent.HANDLED_EVENT);
        addPlan("rmit.ai.clima.jackagt.plans.PSendPlayerToDepot",0);
        addPlan("rmit.ai.clima.jackagt.plans.PTellPlayerToMove",0);
        addPlan("rmit.ai.clima.jackagt.plans.PSendPlayerToGold",0);
        addPlan("rmit.ai.clima.jackagt.plans.PTellPlayerToDrop",0);
        addPlan("rmit.ai.clima.jackagt.plans.PTellPlayerToPick",0);
        addPlan("rmit.ai.clima.jackagt.plans.PSendPlayerToExplore",0);
    }
    
    public CResolvePlayerActions(aos.jack.jak.agent.NameSpace outer)
    {
        super(outer);
    }
    
    synchronized public void __bindNames()
    {
        super.__bindNames();
        __bindToPlan("rmit.ai.clima.jackagt.plans.PSendPlayerToDepot");
        __bindToPlan("rmit.ai.clima.jackagt.plans.PTellPlayerToMove");
        __bindToPlan("rmit.ai.clima.jackagt.plans.PSendPlayerToGold");
        __bindToPlan("rmit.ai.clima.jackagt.plans.PTellPlayerToDrop");
        __bindToPlan("rmit.ai.clima.jackagt.plans.PTellPlayerToPick");
        __bindToPlan("rmit.ai.clima.jackagt.plans.PSendPlayerToExplore");
    }
    
    private rmit.ai.clima.jackagt.data.BPlayerPosition __named_data_bel_playerTarget_dat()
    {
        bel_playerTarget_dat = (rmit.ai.clima.jackagt.data.BPlayerPosition) getNamedObject("bel_playerTarget_dat","rmit.ai.clima.jackagt.data.BPlayerPosition");
        return bel_playerTarget_dat;
    }
    
    private rmit.ai.clima.jackagt.data.BPlayerPosition __named_data_bel_playerPositions_dat()
    {
        bel_playerPositions_dat = (rmit.ai.clima.jackagt.data.BPlayerPosition) getNamedObject("bel_playerPositions_dat","rmit.ai.clima.jackagt.data.BPlayerPosition");
        return bel_playerPositions_dat;
    }
    
    private rmit.ai.clima.jackagt.data.BPlayerGold __named_data_bel_playerGold_dat()
    {
        bel_playerGold_dat = (rmit.ai.clima.jackagt.data.BPlayerGold) getNamedObject("bel_playerGold_dat","rmit.ai.clima.jackagt.data.BPlayerGold");
        return bel_playerGold_dat;
    }
    
    private rmit.ai.clima.jackagt.data.BGoldAt __named_data_bel_goldAt_dat()
    {
        bel_goldAt_dat = (rmit.ai.clima.jackagt.data.BGoldAt) getNamedObject("bel_goldAt_dat","rmit.ai.clima.jackagt.data.BGoldAt");
        return bel_goldAt_dat;
    }
    
    private rmit.ai.clima.jackagt.data.BMoveHint __named_data_bel_moveHint_dat()
    {
        bel_moveHint_dat = (rmit.ai.clima.jackagt.data.BMoveHint) getNamedObject("bel_moveHint_dat","rmit.ai.clima.jackagt.data.BMoveHint");
        return bel_moveHint_dat;
    }
    
    private rmit.ai.clima.jackagt.data.SimulationProp __named_data_bel_simulationProp_dat()
    {
        bel_simulationProp_dat = (rmit.ai.clima.jackagt.data.SimulationProp) getNamedObject("bel_simulationProp_dat","rmit.ai.clima.jackagt.data.SimulationProp");
        return bel_simulationProp_dat;
    }
    
    private rmit.ai.clima.jackagt.data.BPlayerClosestGold __named_data_bel_playerClosestGold_dat()
    {
        bel_playerClosestGold_dat = (rmit.ai.clima.jackagt.data.BPlayerClosestGold) getNamedObject("bel_playerClosestGold_dat","rmit.ai.clima.jackagt.data.BPlayerClosestGold");
        return bel_playerClosestGold_dat;
    }
    
    private rmit.ai.clima.jackagt.data.BObstacleAt __named_data_bel_obstacleAt_dat()
    {
        bel_obstacleAt_dat = (rmit.ai.clima.jackagt.data.BObstacleAt) getNamedObject("bel_obstacleAt_dat","rmit.ai.clima.jackagt.data.BObstacleAt");
        return bel_obstacleAt_dat;
    }
    
}
