/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*
  This code is generated by JAC version 5.4 by
  Agent Oriented Software. http://www.agent-software.com.au

    DO NOT ALTER THIS CODE AND DO NOT REMOVE THIS COMMENT
 *=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
package rmit.ai.clima.jackagt.events;
import aos.jack.jak.event.BDIGoalEvent;

/******** Start PDT Design Block *** DO NOT EDIT IT *********/

public class EIdentifyTarget extends aos.jack.jak.event.BDIGoalEvent {
    public java.lang.String playerName;
    private final static java.lang.String[] __eventVariableNames = {
            "playerName"};
    private final static java.lang.String[] __eventVariableTypes = {
            "String"};
    public java.lang.String getDocumentation()
    {
        return "/******** Start PDT Design Block *** DO NOT EDIT IT *********/\n";
    }
    
    public rmit.ai.clima.jackagt.events.EIdentifyTarget post(java.lang.String name)
    {
        rmit.ai.clima.jackagt.events.EIdentifyTarget tmp = new rmit.ai.clima.jackagt.events.EIdentifyTarget();
        tmp.init(this);
        return tmp.post_body(name);
    }
    
    private rmit.ai.clima.jackagt.events.EIdentifyTarget post_body(java.lang.String name)
    {
        this.playerName = name;
        return this;
    }
    
    public EIdentifyTarget()
    {
    }
    
    public java.lang.String stateInfo()
    {
        return "rmit/ai/clima/jackagt/events/EIdentifyTarget.event [" + super.stateInfo() + "]";
    }
    
    public java.lang.String[] variableNames()
    {
        return __eventVariableNames;
    }
    
    public java.lang.String[] variableTypes()
    {
        return __eventVariableTypes;
    }
    
    public java.lang.Object getVariable(int n)
    {
        switch (n) {
            case 0: 
            {
                return aos.util.ToObject.box(playerName);
            }
            default: 
            {
                throw new java.lang.IndexOutOfBoundsException("Event " + this + " does not have variable number " + n);
            }
        }
    }
    
}
